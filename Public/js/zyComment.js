	...
	$.fn.zyComment = function(options,param){
		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				html += '	<div class="text" style="font-size:2rem;padding-bottom:10px;border-bottom: 1px solid #DFDFDF;"> 所有评论 </div>';
				html += '</div>';
				$(self).append(html);
				...
				$.each(para.agoComment, function(k, v){
					...
					item += '	<a class="avatar">';
					item += '		<img src="http://shop.sweetencounter.cn/foot.png">';
					item += '	</a>';
					...
					item += '			<span class="reply myreply" onclick="getid(this)"  data-id="'+v.id+'" selfID="'+v.id+'" >回复</span>';
					item += '		</div>';
					item += '	</div>';
					item += '</div>';
			...
			this.createFormCommentHtml = function(){
				...
				boxHtml += '				<label class="userNameLabel" for="userName">need not write</label>';
				boxHtml += '			</div>';
				boxHtml += '			<div class="field" >';
				boxHtml += '				<input type="text" id="userEmail" />';
				boxHtml += '				<label class="userEmailLabel" for="userName">need not write</label>';
				boxHtml += '			</div>';
				boxHtml += '		</div>';
				boxHtml += '		<div class="contentField field" >';
				boxHtml += '			<textarea id="commentContent"></textarea>';
				boxHtml += '			<label class="commentContentLabel" for="commentContent">输入评论内容</label>';
				boxHtml += '		</div>';
				boxHtml += '		<div id="submitComment" onclick="newReply();" class="ui button teal submit labeled icon">';
			...
			this.replyClickEvent = function(){
				...
				$(self).find(".actions .reply").live("click", function(){
					...
					// 3.添加取消回复按钮
					$(this).parent(".actions").append('<span class="cancel" href="javascript:void(0)">取消回复</span>');
			...
			this.addReplyCommentFrom = function(id){
				...
				boxHtml += '		</div>';
				boxHtml += '		<div id="publicComment" onclick="reply();" class="ui button teal submit labeled icon">';
				boxHtml += '			<i class="icon edit"></i> 提交评论';
				boxHtml += '		</div>';
				boxHtml += '	</div>';
				boxHtml += '</form>';
			...
			this.addNewComment = function(param){
				...
				item += '		<div class="actions">';
				item += '			<span class="reply myreply"  onclick="getid(this)" data-id="'+param.id+'" selfID="'+param.id+'" >回复</span>';
				item += '		</div>';
				item += '	</div>';
				item += '</div>';
